...
// if (ontology == null) {
// Map<String, Object> ontologyPm =
// ServiceUtils.ontologyAssetsSetup("act_ont_015.owl", manager);
...
// final OpenlletReasoner reasoner =
// OpenlletReasonerFactory.getInstance().createReasoner(ontology);
...
// OWLClass targetActivityClass = manager.getOWLDataFactory().getOWLClass(":" +
// query.get("activity"), pm);
...
// Set<OWLAxiom> satisfiedAxioms = explainer.getInstanceExplanation(ind,
// targetActivityClass);
...
// }
// if (rule == null) {
// System.out.println("The sample is not an instance of " +
// query.get("activity") + " activity");
// return null;
// }
...
// List<ActionEventConstraintPojo> entailedConstraints =
// OwlToPojo.getConstraints(axiomAtoms);
...
// String eventName =
// axiom2.getClassExpression().asOWLClass().getIRI().getShortForm();
// if (entailedEvents.contains(eventName)) {
// String eventIndividualName =
// axiom2.getIndividual().asOWLNamedIndividual().getIRI().getShortForm();
...
// String propName =
// axiom2.getProperty().asOWLDataProperty().getIRI().getShortForm();
// if (propName.equals(Predicate.HAS_START_TIME)) {
// String eventInstanceName =
// axiom2.getSubject().asOWLNamedIndividual().getIRI().getShortForm();
...
// boolean eSatisfied = this.isEventSatisfied(event, activityIndividual,
// manager, ontology, pm);
// if (!eSatisfied) {
...
// OwlToPojo.getConstraints(rule.bodyList());
// for (ActionEventConstraintPojo constraint : constraints) {
// boolean cSatisfied = this.isContraintSatisfied(constraint,
// activityIndividual, manager,
// ontology, pm);
// if (!cSatisfied) {
...
// // private methods
// private boolean isEventSatisfied(String eventType, OWLNamedIndividual ind,
// OWLOntologyManager manager,
// OWLOntology ontology,
// PrefixManager pm) {

// // add the single event activity into the ontology
// OWLClass singleEventClass = manager.getOWLDataFactory().getOWLClass(":" +
// ActivityList.SINGLE_EVENT_ACTIVITY,
// pm);
// OWLClass rootActivityClass = manager.getOWLDataFactory().getOWLClass(":" +
// ActivityList.ROOT_ACTIVITY, pm);
// OWLAxiom subclassAxiom =
// manager.getOWLDataFactory().getOWLSubClassOfAxiom(singleEventClass,
// rootActivityClass);
...
// private boolean isContraintSatisfied(ActionEventConstraintPojo constraint,
...
// // event predicate
// bodyString += String.format("^%s(a,e%s)^%s(e%s)", Predicate.HAS_EVENT, i,
// event, i);
// // start and end times
// if (constraint.getTh1() >= 0) {
// bodyString += String.format("^%s(e%s,t1_e%s)", Predicate.HAS_START_TIME, i,
// i);
// }
...
// // check the rule entailment //TODO: internal reasoner might not be a good
// ideal
// final OpenlletReasoner reasoner =
// OpenlletReasonerFactory.getInstance().createReasoner(ontology);
// final PelletExplanation explainer = new PelletExplanation(reasoner);
...
// // explanation = String.format(explanation + "%s more than %s seconds and
// less
// // than %s seconds.",
// // axiomEvents.get(0).getType(),
// // axiomConstraint.getMinValue(),
